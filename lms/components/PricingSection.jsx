...
const ACCENT_OPTIONS = [
  '#185FA5', '#7c3aed', '#e11d48', '#059669', '#d97706', '#0284c7',
];
...
function PriceCounter({ value }) {
  ...
  useEffect(() => {
    ...
    gsap.to({ val: from }, {
      val: to, duration: .7, ease: 'power2.out',
...
function PricingCard({ plan, isYearly, lang }) {
    ...
    >
      ...
      <div style={{
        ...
        {isSchool ? (
          ...
          <>
            <span style={{ fontSize: '.65rem', fontWeight: 700, opacity: .85 }}>£</span>
      ...
      {isSchool ? (
        <a
          ...
          style={{
            ...
            fontSize: '.9rem', borderRadius: 12, textAlign: 'center',
            textDecoration: 'none', border: '2px solid rgba(0,0,0,.1)',
            boxShadow: '4px 4px 0 rgba(0,0,0,.1)',
            transition: 'transform .15s, box-shadow .15s',
          }}
          onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '6px 6px 0 rgba(0,0,0,.12)'; }}
          onMouseLeave={e => { e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = '4px 4px 0 rgba(0,0,0,.1)'; }}
        >
          💬 {lang === 'ar' ? 'تواصل معنا للحصول على عرض' : 'Contact Us for a Quote'}
        </a>
      ) : (
        <a
          href={plan.checkout_url || WHATSAPP_HREF}
          target="_blank"
          rel="noopener noreferrer"
          style={{
            display: 'block', width: '100%', padding: '11px 16px',
...
export default function PricingSection() {
  ...
  const hasYearly = filtered.some(p => p.price_yearly > 0 && p.price_yearly !== p.price_monthly);
    ...
    <section ref={sectionRef} style={{
      ...
      <div className="container">
          ...
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 12, marginBottom: 36 }}>
            ...
            <button
              ...
              style={{
                ...
                background: isYearly ? 'var(--primary)' : '#cbd5e1',
            ...
            {isYearly && (
              <span style={{
                background: '#dcfce7', color: '#166534', fontSize: '.78rem', fontWeight: 800,
                padding: '3px 10px', borderRadius: 99, border: '1px solid #bbf7d0',
              }}>
          ...
          <div className="pricing-grid-inner" style={{
            ...
            {filtered.map(plan => (
              <div key={plan.id} className="pricing-card-item">
                <PricingCard plan={plan} isYearly={isYearly} lang={lang} />
              </div>
            ))}
          </div>
        )}
        ...
        <p style={{ textAlign: 'center', marginTop: 32, fontSize: '.8rem', color: '#94a3b8' }}>
          {lang === 'ar'
            ? '* الأسعار بالجنيه الإسترليني · جميع الباقات تشمل الدعم الفني · يمكن الإلغاء في أي وقت'
            : '* Prices in GBP · All plans include support · Cancel anytime'}
        </p>
      </div>
    </section>
  );
