...
// يرفع الملف مباشرة إلى Supabase Storage عبر مسار الرفع المخصص ويُرجع رابطاً
// عاماً قصيراً — بدل تحويله إلى base64 وتضخيم صف قاعدة البيانات (القاعدة
// المعتمدة في المشروع لكل صور/وسائط الألعاب، راجع CLAUDE.md القسم 9).
async function uploadMedia(file, kind) {
  const body = new FormData();
  body.append('file', file);
  const res  = await fetch(`/api/bogga/lexicon/upload?kind=${kind}`, { method: 'POST', body });
...
export default function LexiconAdminPage() {
  ...
  async function pickImage(e) {
    ...
    setImageUploading(true); setMsg(null);
    try {
      const url = await uploadMedia(f, 'image');
      setImageFile(f); setImageData(url); setClearImage(false);
    } catch (err) {
      setMsg({ type: 'error', text: err.message || 'فشل رفع الصورة' });
    } finally {
  ...
  async function pickAudio(e) {
    ...
    setAudioUploading(true); setMsg(null);
    try {
      const url = await uploadMedia(f, 'audio');
      setAudioFile(f); setAudioData(url); setClearAudio(false);
    } catch (err) {
      setMsg({ type: 'error', text: err.message || 'فشل رفع الصوت' });
    } finally {
              ...
              <div className="card" style={{ position: 'sticky', top: 80 }}>
                ...
                <form onSubmit={handleSave}>
                  ...
                  <div className="form-group" style={{ marginTop: 8 }}>
                    ...
                    <div onClick={() => !imageUploading && imageRef.current?.click()}
                      style={{
                        border: `2px dashed ${imageFile ? '#1a7c40' : 'var(--border)'}`,
                        borderRadius: 10, padding: '10px 14px', cursor: imageUploading ? 'wait' : 'pointer',
                        textAlign: 'center', background: imageFile ? '#e8f8ef' : 'var(--bg)',
                        fontSize: '.83rem', color: imageFile ? '#1a7c40' : 'var(--muted)', fontWeight: 700,
                      }}>
                      {imageUploading ? '⏳ جارٍ الرفع...' : imageFile ? `✅ ${imageFile.name}` : curImg ? '🔄 استبدال الصورة' : '📷 رفع صورة'}
                    </div>
                    <input ref={imageRef} type="file" accept="image/*"
                      onChange={pickImage} style={{ display: 'none' }} disabled={imageUploading} />
                  ...
                  <div className="form-group">
                    ...
                    <div onClick={() => !audioUploading && audioRef.current?.click()}
                      style={{
                        border: `2px dashed ${audioFile ? '#1a7c40' : 'var(--border)'}`,
                        borderRadius: 10, padding: '10px 14px', cursor: audioUploading ? 'wait' : 'pointer',
                        textAlign: 'center', background: audioFile ? '#e8f8ef' : 'var(--bg)',
                        fontSize: '.83rem', color: audioFile ? '#1a7c40' : 'var(--muted)', fontWeight: 700,
                      }}>
                      {audioUploading ? '⏳ جارٍ الرفع...' : audioFile ? `✅ ${audioFile.name}` : curAudio ? '🔄 استبدال الصوت' : '🎤 رفع ملف صوتي'}
                    </div>
                    <input ref={audioRef} type="file" accept="audio/*"
                      onChange={pickAudio} style={{ display: 'none' }} disabled={audioUploading} />
                  ...
                  <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
                    <button type="submit" className="btn btn-accent" disabled={saving || imageUploading || audioUploading} style={{ flex: 1 }}>
                      {saving
