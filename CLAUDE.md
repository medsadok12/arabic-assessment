...
### عاجل
- [ ] إضافة `RESEND_API_KEY` في Vercel → aarem-lms → Environment Variables
- [ ] Redeploy بعد إضافة المفتاح
- [ ] ربط نطاق مخصص بـ Resend (بعد شراء الدومين)
- [ ] إعداد Azure TTS: إنشاء Speech service → إضافة `AZURE_SPEECH_KEY` + `AZURE_SPEECH_REGION`
